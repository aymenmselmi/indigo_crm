...
export function normalizeActivity(act) {
  ...
  const contactName = act.relatedContact
    ? [act.relatedContact.firstName, act.relatedContact.lastName].filter(Boolean).join(' ') || act.relatedContact.email
    : null;
  const oppName = act.relatedOpportunity?.name || null;
  const linkedParts = [contactName, oppName].filter(Boolean);
  return {
    id:          act.id,
    kind:        act.type,
    ...
    linkedLabel: linkedParts.length ? linkedParts.join(' · ') : null,
    status:      act.status,
