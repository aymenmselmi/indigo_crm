...
export const AccountsView = ({ openDetail, addToast }) => {
  ...
  const rows = accounts.filter(a => {
    ...
    if (filter === 'strategic' && a.tier !== 'Strategic') return false;
    if (filter === 'at-risk' && a.health === 'good') return false;
    return true;
  });
  ...
  return (
    <>
      <div className="filters">
        ...
        <button className={`filter-pill ${filter === 'strategic' ? 'active' : ''}`} onClick={() => setFilter('strategic')}>
          Strategic
        </button>
        <button className={`filter-pill ${filter === 'at-risk'   ? 'active' : ''}`} onClick={() => setFilter('at-risk')}>
          <span className="dot" style={{ width: 6, height: 6, borderRadius: '50%', background: 'var(--danger)' }}></span>At-risk
        </button>
